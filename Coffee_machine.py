...
def order(ask):
    failure="Sorry there is not enough water."
    asks=MENU[ask]
    if ask=="off":
        serve=False
        exit()
        ...
        return resources
    for i in asks["ingredients"]:
        if asks["ingredients"][i]<=resources[i]:
            resources[i]-=asks["ingredients"][i]
        else:
            return failure
